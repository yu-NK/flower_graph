...
def junk(img_skel,img,img_num):
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/02-2_junktion-mark_onSkel/ORA-Coronal-{0:03d}_junktion-mark_onSkel.png'.format(img_num),skel_b)
...
def skel(img_num):
    ...
    #Original
    img_col = cv2.imread('Input/ORA-Coronal_y_part/ORA-Coronal-{0:03d}_part.png'.format(img_num))

    #NLMDを用いた2値化
    img = cv2.imread('Input/ORA-Coronal_y_part/ORA-Coronal-{0:03d}_part.png'.format(img_num),0)
    img = cv2.fastNlMeansDenoising(img,h=6)
    th_img = np.where(img>50 ,1 ,0)
    
    #img_NLMD_50 = np.where(img>50 ,img_NLMD,0)
    #dst = cv2.Canny(img_NLMD_50, 150, 400)
    #th_img = th_img - dst
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/02-1_skeleton/ORA-Coronal-{0:03d}_skeleton.png'.format(img_num),skel_b)

    #細線化画像による分岐点探索
    temp_j,img_j,img_skel = junk(skel_b,img_col,img_num)
    
    cv2.imwrite('Output/Spectral_sys_Coronal/02-3_junction-mark_onIMG/ORA-Coronal-{0:03d}_junction-mark_onIMG.png'.format(img_num),img_j)
...
for img_num in range(481,800):
    ...
    #img_NLMD_50 = np.where(img_NLMD>50 ,img_NLMD,0)
    
    #dst = cv2.Canny(img_NLMD_50, 150, 400)
    
    #th_img = th_img - dst
    
    cv2.imwrite('Output/Spectral_sys_Coronal/00-1_NLMD-h6/ORA-Coronal-{0:03d}_NLMD-h6.png'.format(img_num),img_NLMD)
    #cv2.imwrite('Output/Spectral_sys_Coronal/00-2_NLMD-h6-50/ORA-Coronal-{0:03d}_NLMD-h6-50.png'.format(img_num),img_NLMD_50)
    #cv2.imwrite('Output/Spectral_sys_Coronal/00-3_NLMD-h6-50-Canny/ORA-Coronal-{0:03d}_NLMD-h6-50-Canny.png'.format(img_num),th_img)
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/01_k-means/ORA-Coronal-{0:03d}_k-means-{1:03d}.png'.format(img_num,k_c),img_col)
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/03-1_junk-del/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-del.png'.format(img_num,k_c),img_col)

    cv2.imwrite('Output/Spectral_sys_Coronal/03-2_junk-only/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-only.png'.format(img_num,k_c),del_cluster)
    
    now = datetime.datetime.now()
    print(now)
    print('分岐点を除去した細線化をラベリング')
    
    img_col_skel = cv2.imread('Input/ORA-Coronal_y_part/ORA-Coronal-{0:03d}_part.png'.format(img_num))
    skel_col = cv2.imread('Input/ORA-Coronal_y_part/ORA-Coronal-{0:03d}_part.png'.format(img_num))
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/04-1_skel-label_onIMG/ORA-Coronal-{0:03d}_skel-label_onIMG.png'.format(img_num),img_col_skel)
    cv2.imwrite('Output/Spectral_sys_Coronal/04-2_skel-label_onSkel/ORA-Coronal-{0:03d}_skel-label.png'.format(img_num),skel_col)
    
    now = datetime.datetime.now()
    print(now)
    print('分岐点周辺を削除した画像を2値化')
    
    th_img_clu = cv2.imread('Input/ORA-Coronal_y_part/ORA-Coronal-{0:03d}_part.png'.format(img_num),0)
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/05_junk-del-th/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-del-th.png'.format(img_num,k_c),th_img_clu)
    ...
    #ラベリング結果を保存
    cv2.imwrite('Output/Spectral_sys_Coronal/06-1_junk-del-label/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-del-label.png'.format(img_num,k_c),img_col)
    #ノイズ除去した画像を保存
    cv2.imwrite('Output/Spectral_sys_Coronal/06-2_junk-del-label-th/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-del-label-th.png'.format(img_num,k_c),label_th)
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/07_junk-del-label-inte/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-del-label-inte.png'.format(img_num,k_c),img_col)
    ...
    cv2.imwrite('Output/Spectral_sys_Coronal/08_junk-del-label-inte-sp/ORA-Coronal-{0:03d}_k-means-{1:03d}_junk-del-label-inte_sp.png'.format(img_num,k_c),img_col)
